...
public class GLRectView extends GLView {
	...
	private HashMap< Integer, ArrayList<GLRenderParams> > mReanderList = new HashMap<>();
...
//				-screenHeight * ty / yDpi, tz);
	...
	protected void addRender(GLRenderParams render) {
		mRenders.add(render);
		ArrayList<GLRenderParams> list = mReanderList.get(render.getType() );
		if( list != null ){
			list.add( render );
		}else{
			list = new ArrayList<GLRenderParams>();
			list.add(render);
			mReanderList.put( render.getType(), list);
		}
