...
public class GLProcessView extends GLGroupView {
	private GLProcessMode mGlProcessMode = GLProcessMode.RECTANGLE; //默认长方形
	 ...
	 * @return void
	 */
	public GLProcessView(Context context, GLProcessMode mode) {
		super(context);
	 ...
	 */
	public void init() {
		if (mGlProcessMode == GLProcessMode.RECTANGLE) {
	...
	private void setPosition(GLRectView view, float left, float top, float right, float bottom) {
		if (view != null) {
			if (view.getAlign() == null
					...
					|| view.getAlign().equals(GLAlign.TOP)) { //left, top
	...
	private void setLayoutParams() {
		...
		if (mGlProcessMode == GLProcessMode.CIRCLE) {
