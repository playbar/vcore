...
import com.bfmj.viewcore.render.GLVideoRect.TextureType;
...
//		 }
	...
	public void setPlayMode(int mode){
		...
		if (mode == MODE_2D && mRenderParams != null){
			mRenderParams.setTextureType(TextureType.TEXTURE_TYPE_ALL);
		}
	}
...
//		mSurfaceTexture.setOnFrameAvailableListener(this);
	...
	public void draw(boolean isLeft) {		
		...
		if (mPlayMode == MODE_3D_LEFT_RIGHT_FULL || mPlayMode == MODE_3D_LEFT_RIGHT_HALF){
			...
			if (isLeft){
				mRenderParams.setTextureType(TextureType.TEXTURE_TYPE_LEFT);
			} else {
				mRenderParams.setTextureType(TextureType.TEXTURE_TYPE_RIGHT);
			}
		} else {
			mRenderParams.setTextureType(TextureType.TEXTURE_TYPE_ALL);
		}
